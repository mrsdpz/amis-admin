...
这里我们使用接收适配器在请求成功后设置 cookie,最终我们的登录页面表单代码如下:
        ...
        api: {
            ...
            adaptor: function (payload, response) {
                if (payload.status === 0) {
                    ...
                    document.cookie = `username=${payload.data.username};${expires}; path=/`;
                }
                return payload
            },
        },
...
同样使用请求适配器即可在接口发起请求前校验cookie 中是否存在`amisToken`

但是这里我们使用另外一种方式,使用 express 的中间件来校验 cookie,在后面的文章会使用到这个中间件.
